...
export default function EnhancedTable({
  ...
  return (
    <div className="w-full flex flex-col gap-4">
              ...
              >
                {showActions && (
                  <td className="px-4 py-2 space-x-2">
                    ...
                    <button onClick={(e) => { e.stopPropagation(); onDelete(item[idField]); }} className="text-red-600 hover:underline">
                      <Trash2 size={16} />
                    </button>
                  </td>
                )}
                {columns.map((col) => (
                  <td key={col.key} className="px-4 py-2 text-gray-900 min-w-[200px] max-w-[300px] break-words truncate">
      ...
      {/* Paginação */}
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mt-4 text-sm gap-2 text-gray-700">
        <div className="flex items-center">
          <span>Itens por página:</span>
          <select
            value={rowsPerPage}
            onChange={(e) => onRowsPerPageChange(parseInt(e.target.value))}
            className="ml-2 border border-gray-300 bg-white text-gray-800 px-2 py-1 rounded"
          >
